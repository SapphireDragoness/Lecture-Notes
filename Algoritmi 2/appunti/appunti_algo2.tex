...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        ...
        \For{ogni nodo $u$ di $G$}
            \If{color[$u$] = white \textit{and} VISITA RICORSIVA CICLO(G,u)}
...
\begin{itemize}
    ...
    \item è sntisimmetrica: se $v$ è raggiungibile da $u$ ed $u$ è raggiugibile da $v$, allora $v$ e $u$ sono coincidenti
...
\begin{enumerate}
    ...
    da $y$ a $x$, altrimenti $x$ sarebbe nel sottoalbero di $y$. Quindi, non esisterà il cammino da $x$ a $y$ in $G^T$ 
    ($d[y]<f[y]<d[x]<f[x]$).
\end{enumerate}
...
\begin{proof}
    ...
    Consideriamo un cammino sull'albero $A^T$ a partire dalla radice $u$; sia $v$ il primo vertice sul cammino per cui il 
...
di tipo \textit{greedy} e di \textit{programmazione dinamica}.
...
\begin{enumerate}
    ...
    \item ottieni il sottoproblema isultante dall'eliminazione della variabile al punto 1
...
stesso. Ad ogni ciclo, scelgo un vertice $u$ da aggiungere all'albero tra quelli non ancora inseriti ma aggiunti dalla 
...
\begin{proprietà}[3]
    ...
    \begin{proof}
        ...
        $d[u]=d[v_k]=d[v_{k-1}]+W(v_{k-1},v_k)$. Ma $W(v_{k-1},v_k)<\infty$ e $d[u]=\infty$, quindi $d[v_{k-1}]=\infty$.
...
\begin{proof}
    \textit{Caso base}. Il predicato è vero perchè all'inizio $S$ è vuoto.
