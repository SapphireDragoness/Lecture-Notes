...
Una \textbf{sequenza di scheduling}, o semplicemente \textbf{scheduling}, è una sequenza di lavori $L_1,L_2,\dots,L_n$ che 
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \If{$x=y$}\\
            \Return{$x$}
        \EndIf
        \Return{PATH RECONSTRUCTION(P,x,P[x,y])+y}
    \end{algorithmic}
\end{algorithm}
...
\begin{itemize}
    ...
    \item \textbf{Problemi di ricerca}: data l'istanza di un problema, restituire una soluzione ammissibile.
    \item \textbf{Problemi di ottimizzazione}: dat l'istanza di un problema, restituire una soluzione ottima secondo un 
    determinato criterio.
\end{itemize}
\subsection{Classi di complessità}
Una \textbf{classe di complessità} è un insieme di problemi che possono essere risolti usando le stesse risorse di calcolo.
Dato $n$, che indica la dimensione dell'input, possiamo classificare i problemi in base al tempo o allo spazio richiesti:
\begin{itemize}
    \item \textbf{TIME(f(n))}: è l'insieme dei problemi che possono essere risolti in tempo $\mathcal{O}(f(n))$
    \item \textbf{SPACE(f(n))}: è l'insieme dei problemi che possono essere risolti in spazio $\mathcal{O}(f(n))$
\end{itemize}
\subsubsection{Classe P}
Per semplicità, le classi di complessità sono raggruppate nelle cosiddette \textbf{classi unione}.
...
\textbf{Espressione in Forma Normale Congiuntiva}: congiunzione di disgiunzioni (\textbf{clausole}) di variabili booleane
(\textbf{letterali}). Ad esempio
\begin{equation*}
...
Un \textbf{certificato} è un oggetto $y$, dipendente dall'istanza $x$ e dal problema $P$, che possa giustificare 
$(x,1)\in P$ (per i problemi decisionali).
Gli algoritmi possono essere riscritti in due fasi: fase di \textbf{costruzione} e fase di \textbf{verifica}.
...
Gli algoritmi sviluppati con funzioni di questo tipo sono detti \textbf{non deterministici}, perchè la loro computazione 
...
\textbf{Formula Booleana Quantificata}: è un espressione in forma normale congiuntiva preceduta da una serie di quantificatori 
...
\begin{itemize}
    \item \textbf{Problemi NP-hard}: un problema decisionale $D$ si definisce NP-hard se ogni problema $Q\in NP$ è riconducibile
    a $D$ in tempo polinomiale 
    \item \textbf{Problemi NP-completi}: un problema decisionale $D$ si definisce NP-completo se è NP-hard ed appartiene 
    alla classe $NP$.
\end{itemize}
...
per un problema è ottima. Privilegiando l'efficienza a costo dell'ottimalità, si utilizzano \textbf{algoritmi di approssimazione}
...
Dato un grafo pesato, completo e non orientato bisogna trovare un \textbf{ciclo Hamiltoniano} di costo minimo. Il problema 
...
Una tecnica generale per trovare soluzioni buone è quella della \textbf{ricerca locale}. In un problem di ottimizzazione: 
\begin{enumerate}
